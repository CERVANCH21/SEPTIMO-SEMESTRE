...
def process_input():
    ...
    for line_num, line in enumerate(lines, start=1):
        ...
        # Asignación de tipo de datos (iron, tars, cas)
        if line.startswith(("entier", "reel", "chaine")):
            ...
            tokens = re.findall(r"[\+\-\*/%,=;:]|[A-Z][a-zA-Zá-úÁ-Ú1-5_@]*|[\"'][^\"']*[\"']", parts[1])
            for token in tokens:
                if SEPARATOR_PATTERN.match(token):  # Es un separador (coma, punto y coma, etc.)
                    ...
                    if VAR_PATTERN.match(token):  # Validar con la expresión regular
                        ...
                        #Mostrar mensaje de "Identificador inválido" en lugar de agregar a errores
                        messagebox.showerror("Error", f"Identificador inválido: '{token}' en la línea {line_num}")
        ...
        # Procesamiento de asignaciones (a = valor) y operaciones
            ...
            if var not in data_types:
                ...
                error_count += 1
                # Agregar la variable no definida a la tabla de lexemas (con valor vacío)
                ...
                for i, token in enumerate(tokens):
                    if SEPARATOR_PATTERN.match(token):  # Es un operador, coma, punto y coma, etc.
                        ...
                        if token_type is not None:
                            if expected_type == "chaine" and token_type != "chaine":
                            ...
                            #errors.append([f"Err{error_count}", token, line_num, "Variable no definida"])
                ...
                if  error_tokens:
                    errors.append([f"ER{error_count}", ", ".join(error_tokens), line_num, f"Incompatibilidad de tipos({expected_type})"])
                    error_count += 1
...
#text_area = scrolledtext.ScrolledText(ventana, width=50, height=10)
#text_area.pack(pady=10)

# Botón para procesar entrada
#btn_process = tk.Button(ventana, text="Procesar", command=process_input)
#btn_process.pack(pady=5)

# Marco para la tabla de lexemas
#frame_lexemas = ttk.Frame(ventana)
#frame_lexemas.pack(pady=5, fill=tk.BOTH, expand=True, padx=10)  # Añadir padx para alinear con el borde

# Tabla de lexemas con barra de desplazamiento
#table_lexemas = ttk.Treeview(frame_lexemas, columns=("Lexema", "Tipo de dato"), show="headings")
#table_lexemas.heading("Lexema", text="Lexema")
#table_lexemas.heading("Tipo de dato", text="Tipo de dato")
#table_lexemas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

# Barra de desplazamiento vertical para la tabla de lexemas
#scrollbar_lexemas = ttk.Scrollbar(frame_lexemas, orient=tk.VERTICAL, command=table_lexemas.yview)
#scrollbar_lexemas.pack(side=tk.RIGHT, fill=tk.Y)
#table_lexemas.configure(yscrollcommand=scrollbar_lexemas.set)

# Marco para la tabla de errores
#frame_errores = ttk.Frame(ventana)
#frame_errores.pack(pady=5, fill=tk.BOTH, expand=True, padx=10)  # Añadir padx para alinear con el borde

# Tabla de errores con barra de desplazamiento
#table_errores = ttk.Treeview(frame_errores, columns=("Token", "Lexema", "Renglón", "Descripción"), show="headings")
#table_errores.heading("Token", text="Token")
#table_errores.heading("Lexema", text="Lexema")
#table_errores.heading("Renglón", text="Renglón")
#table_errores.heading("Descripción", text="Descripción")
#table_errores.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

# Barra de desplazamiento vertical para la tabla de errores
#scrollbar_errores = ttk.Scrollbar(frame_errores, orient=tk.VERTICAL, command=table_errores.yview)
#scrollbar_errores.pack(side=tk.RIGHT, fill=tk.Y)
#table_errores.configure(yscrollcommand=scrollbar_errores.set)
...
tabla_errores = ttk.Treeview(frame_errores, columns=("Token error", "Lexema", "Renglón", "Descripción"), show="headings")
